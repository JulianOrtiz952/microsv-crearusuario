...
class DeliveryConsumer:
    """
    Consumidor de eventos de RabbitMQ para el servicio de entregas
    Escucha eventos de customer.created y crea envíos pendientes de paquetes de bienvenida
    """
    
    def __init__(self):
        self.rabbitmq_url = settings.RABBITMQ_URL
        self.exchange_name = settings.CUSTOMERS_EXCHANGE
        self.queue_name = settings.DELIVERY_QUEUE
    ...
    def handle_customer_created(self, customer_data):
        ...
        Crea un envío pendiente del paquete de bienvenida
        """
        try:
            ...
            logger.info(
                f"Envío de paquete de bienvenida creado para cliente: {customer_name} "
                f"(ID: {customer_id}) - Envío ID: {delivery.id} - Estado: PENDIENTE"
            )
            
        except Exception as e:
            logger.error(f"Error creando envío: {str(e)}")
            raise
